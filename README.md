# A Calculator for your Chat Agent
...
You can easily run this calculator as an MCP server, or call it from the OpenAI dotnet library (or Semantic Kernel/Agent Framework/etc) as a function tool.
...
To use it in code, just call the static Calculate method like this:
...
Note that in 2026, many coding agents will simply just run a python command to do math, so in those cases this tool can save a few thousand reasoning tokens but won't change the result. However, if you're wanting caclulations for an LLM in a non-dev environment this will still work like a champ. If you want to run the MCP server over HTTP you will need to change the Program.cs statup command, though probably in the next update I'll make it a command line option.
